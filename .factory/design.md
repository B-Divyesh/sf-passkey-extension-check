...
- `ink-muted` #536562 — secondary copy (5.4:1 on paper).
- `teal` #0A665F — action/verified ink (5.6:1 on paper).
- `teal-deep` #064842 — pressed/focus companion.
- `saffron` #D89012 — caution tabs, always paired with a label/icon.
...
Generated with the factory image deployment through `/opt/fleet/lib/gen-image.sh` on 2026-08-27. The output is original project artwork. Source PNG and prompt sidecar are retained in `assets/src/`; optimized WebP is shipped in the site.
